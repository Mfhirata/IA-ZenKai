...
TAMANHO_BLOCO = 64
ARQUIVO_SAIDA = "blocos_alterados.csv"
...
with open(ARQUIVO_SAIDA, mode='w', newline='') as csvfile:
    ...
    writer.writerow(["Offset", "Original", "Modificado", "Diferença Média"])

print(f"\n📁 Comparando blocos alterados: {arquivos_bin[0]} VS {arquivos_bin[1]}\n")
print("🧠 Blocos alterados detectados:")
...
for offset in range(0, min(len(dados_orig), len(dados_mod)), TAMANHO_BLOCO):
    ...
    if bloco_orig != bloco_mod:
        ...
        # Diferença média absoluta dos bytes
...
print(f"\n✅ CSV de blocos alterados salvo em: {ARQUIVO_SAIDA}")
