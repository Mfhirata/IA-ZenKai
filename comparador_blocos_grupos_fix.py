...
# Estrutura para agrupar por tipo
grupos = {"IGNIÇÃO": [], "INJEÇÃO": [], "LIMITADOR": [], "DESCONHECIDO": []}

# Criar CSV e escrever dentro do mesmo bloco with
with open(ARQUIVO_SAIDA, mode='w', newline='') as csvfile:
    ...
    print(f"\n📁 Comparando blocos alterados: {arquivos_bin[0]} VS {arquivos_bin[1]}\n")
    print("🧠 Blocos alterados detectados e agrupados:")
    ...
    for offset in range(0, min(len(dados_orig), len(dados_mod)), TAMANHO_BLOCO):
        ...
        if bloco_orig != bloco_mod:
            ...
            grupos[rotulo_mod].append((offset, rotulo_orig, diff_media))
...
print("\n📊 Resumo por grupo de mapas alterados:")
for tipo, blocos in grupos.items():
    if blocos:
        offsets = [f"0x{o[0]:X}" for o in blocos]
        print(f"  - {tipo}: {len(blocos)} blocos → Offsets: {offsets}")
